	...
	if (serv_arry != NULL) {
		...
		for (i = 0; i < count; i++) {
            ...
            if (try == 3) { /* 等于3时，sem的初始化已经失败了3次 */
                MSG("WARNING, Can't initializes the unnamed semaphore of service, ignore this element.\n");
                lgw_free(serv_entry);
                continue;
            }
            ...
            if (str != NULL) {
                ...
                MSG("INFO: Found a server name is \"%s\"\n", str);
            } else {
                lgw_gen_string(&serv_entry->info.serv_name, sizeof(serv_entry->info.serv_name));
                MSG("INFO: The server name mustbe configure, generate a random name: %s\n", serv_entry->info.serv_name);
                continue;
            }
            ...
            if (str != NULL) {
				if (!strncmp(str, "semtech", 7)) {
                    ...
                    serv_entry->report->stats_report = (char*)lgw_malloc(sizeof(char) * STATUS_SIZE);
                    serv_entry->report->stat_up = (stat_up_s*)lgw_malloc(sizeof(stat_up_s));
                    serv_entry->report->stat_down = (stat_down_s*)lgw_malloc(sizeof(stat_down_s));
                    serv_entry->report->mx_report = (pthread_mutex_t*)lgw_malloc(sizeof(pthread_mutex_t));
            ...
            MSG("INFO: packets received with a no CRC will%s be forwarded\n", (serv_entry->filter.fwd_nocrc_pkt ? "" : " NOT"));
...
int parsecfg(const char *cfgfile, gw_s* gw) {
    ...
    if (!strncmp(gw->hal.board, "LG301", 5)) {
        ret1 = parse_SX1301_configuration(cfgfile);
    } else if (!strncmp(gw->hal.board, "LG302", 5)) { 
        ret1 = parse_SX130x_configuration(cfgfile);
    } else if (!strncmp(gw->hal.board, "LG308", 5)) { 
        ret1 = parse_SX1301_configuration(cfgfile);
    } else if (!strncmp(gw->hal.board, "LG02", 4)) { 
        ret1 = parse_lg02_configuration(cfgfile);
