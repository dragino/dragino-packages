...
int gwtraf_start(serv_s* serv) {
    ...
    if (lgw_pthread_create_detached(&serv->thread.t_up, NULL, (void *(*)(void *))gwtraf_push_up, serv)) {
...
static void gwtraf_push_up(void* arg) {
	...
	while (!serv->thread.stop_sig) {
		...
		if ((serv->rxpkt_serv->nb_pkt > 0) && (GW.gps.gps_enabled == true)) {
