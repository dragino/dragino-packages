...
int lgw_connect_sx1302(const char * spidev_path) {
    ...
    spi_stat = lgw_spi_r(lgw_spi_target, LGW_SPI_MUX_TARGET_SX1302, sx1302_regs[SX1302_REG_COMMON_VERSION_VERSION].addr, &u);
    if (spi_stat != LGW_SPI_SUCCESS) {
        DEBUG_MSG("ERROR READING CHIP VERSION REGISTER\n");
        return LGW_REG_ERROR;
    }
    if (u != sx1302_regs[SX1302_REG_COMMON_VERSION_VERSION].dflt) {
