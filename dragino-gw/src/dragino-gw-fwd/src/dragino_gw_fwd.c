...
int main(int argc, char *argv[]) {
	...
	while (!exit_sig && !quit_sig) {
		...
		if (lgw_get_trigcnt(&trig_cnt_us) == LGW_HAL_SUCCESS
			&& trig_cnt_us == 0x7E000000) {
...
void thread_jit(void) {
	...
	while (!exit_sig && !quit_sig) {
		...
		jit_result =
			jit_peek(&jit_queue, &current_concentrator_time, &pkt_index);
		if (jit_result == JIT_ERROR_OK) {
			if (pkt_index > -1) {
				jit_result =
					jit_dequeue(&jit_queue, pkt_index, &pkt, &pkt_type);
				if (jit_result == JIT_ERROR_OK) {
					/* update beacon stats */
