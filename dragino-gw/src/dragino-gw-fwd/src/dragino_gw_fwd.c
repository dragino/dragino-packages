...
volatile bool reload_sig = false;/*  application reload configure */
...
static int parse_gateway_configuration(const char *conf_file) {
	...
	for (i = 0; i < serv_count && ic < MAX_SERVERS; i++) {
	    ...
	    if (val2 != NULL)
		snprintf(servers[ic].port_down, sizeof servers[ic].port_down,
			 "%u", (uint16_t) json_value_get_number(val2));
	    if (val3 != NULL)
		...
		} else if (!strncmp(vtype, "mqtt", 4)) {
		    servers[ic].type = mqtt; // TODO: mqtt forward
    ...
    if (str != NULL) {
	if (strncmp(str, "*", 1) != 0) {
	    snprintf(platform, sizeof platform, "%s", str);
	}
...
void usage(char *proc_name) {
    fprintf(stderr,
	    "Usage: %s [-c config_dir] [-l logfile] [-s spi speed in hz] [-d device file]\n",
	    proc_name);
    exit(1);
}
...
void thread_gps(void) {
	...
	while (rd_idx < wr_idx) {
		...
		/***********************
                 * Found UBX sync char *
                 ***********************/
		latest_msg = lgw_parse_ubx(&serial_buff[rd_idx], (wr_idx - rd_idx),
				  &frame_size);
