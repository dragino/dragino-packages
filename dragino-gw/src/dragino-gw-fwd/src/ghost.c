...
void ghost_start(const char *ghost_addr, const char *ghost_port,
	...
	ghost_run = true;
	i = pthread_create(&thrid_ghost, NULL, (void *(*)(void *))thread_ghost,
					   NULL);
	if (i != 0) {
...
static void thread_ghost(void) {
	...
	i = setsockopt(sock_ghost, SOL_SOCKET, SO_RCVTIMEO, (void *)&ghost_timeout,
				   sizeof ghost_timeout);
	if (i != 0) {
	...
	tobecpy(&buff_req[4 + sizeof(gateway_id)], &reference_coord.lon,
			sizeof(double));
	tobecpy(&buff_req[4 + sizeof(gateway_id) + sizeof(double)],
			&reference_coord.lat, sizeof(double));
	...
	while (ghost_run) {			/* send PULL request and record time */
		...
		while ((int)difftimespec(recv_time, send_time) < NODE_CALL_SECS) {	/* try to receive a datagram */
			msg_len =
				recv(sock_ghost, (void *)buff_down, (sizeof buff_down) - 1, 0);
			...
			if ((msg_len < 4 + GHST_MIN_PACKETSIZE)
				...
				LOGGER
					("WARNING: [ghost] ignoring invalid packet len=%d, protocol_version=%d, id=%d\n",
			...
			if (full) {
				...
				memcpy((void *)(buffRX + ghst_bgn * GHST_RX_BUFFSIZE),
					   buff_down + 4, msg_len - 3);
				pthread_mutex_lock(&cb_ghost);
				ghst_bgn = next;
				pthread_mutex_unlock(&cb_ghost);
