	...
	*(uint32_t *) (buff_req + 8) = net_mac_l;
	...
	while (!exit_sig && !quit_sig) {
		...
		while ((int)difftimespec(recv_time, send_time) < keepalive_time) {
			...
			if (val != NULL) {
				txpkt.rf_power =
					(int8_t) json_value_get_number(val) - antenna_gain;
				j = snprintf((json + buff_index), 300 - buff_index,
	...
	while (!exit_sig && !quit_sig) {
        ...
        do {
            ...
            for (i = 0; i < entry->nbpkt; ++i) {
                ...
                /* Note that in this handling some errors can occur the should not occur.              * We changed these from fatal errors to transient errors. 
                 * In most cases the users expect that the systems keeps alive, 
                 * just starts routing the next packet.
                 * */
                switch (p->status) {
                case STAT_CRC_OK:
                    if (!fwd_valid_pkt) {
                ...
                if (p->modulation == MOD_LORA) {
                    ...
                    j = snprintf((char *)(buff_up + buff_index),
                                 TX_BUFF_SIZE - buff_index, ",\"datr\":%u",
                                 p->datarate);
                    if (j > 0) {
                        buff_index += j;
                    } else {
