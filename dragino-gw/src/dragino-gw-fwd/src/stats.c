...
void stats_report() {
    ...
    for (i = 0; i < serv_count; i++) {
	move_up_ack_quality[i] = moveave(move_up_ack_quality[i], ar_up_ack_rcv[i], ar_up_dgram_sent[i]);
    }
    for (i = 0; i < serv_count; i++) {
	move_dw_ack_quality[i] = moveave(move_dw_ack_quality[i], ar_dw_ack_rcv[i], ar_dw_pull_sent[i]);
    }
    for (i = 0; i < serv_count; i++) {
	move_dw_datagram_quality[i] = moveave(move_dw_datagram_quality[i], ar_dw_dgram_acp[i], ar_dw_dgram_rcv[i]);
    }
    for (i = 0; i < serv_count; i++) {
	move_dw_receive_quality[i] = moveave(move_dw_receive_quality[i], ar_dw_ack_rcv[i], ar_dw_pull_sent[i]);
    }
