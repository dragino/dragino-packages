	...
	for (i = 0; i < nb_pkt; ++i) {
		...
		if (p->status == STAT_UNDEFINED || p->modulation == MOD_UNDEFINED ||
			...
			LOGGER ("WARNING: [stats] received packet with unknown status %u (size %u, modulation %u, BW %u, DR %u, RSSI %.1f)\n",
				 p->status, p->size, p->modulation, p->bandwidth, p->datarate,
				 p->rssi);
		...
		case STAT_CRC_OK:
			increment_up(RX_OK);
			LOGGER
				("INFO: [stats] received packet with valid CRC from mote: %08X (fcnt=%u)\n",
				 mote_addr, mote_fcnt);
			if (!fwd_valid_pkt) {
		...
		case STAT_CRC_BAD:
			increment_up(RX_BAD);
			LOGGER
				("INFO: [stats] received packet with bad CRC from mote: %08X (fcnt=%u)\n",
				 mote_addr, mote_fcnt);
			if (!fwd_error_pkt) {
				continue;		/* skip that packet */
			}
		...
		case STAT_NO_CRC:
			...
			LOGGER
				("INFO: [stats] received packet without CRC from mote: %08X (fcnt=%u)\n",
				 mote_addr, mote_fcnt);
			if (!fwd_nocrc_pkt) {
				continue;		/* skip that packet */
			}
		...
		default:
			...
			LOGGER
				("WARNING: [stats] received packet with unknown status %u (size %u, modulation %u, BW %u, DR %u, RSSI %.1f)\n",
				 p->status, p->size, p->modulation, p->bandwidth, p->datarate,
				 p->rssi);
			continue;
		}
...
static double moveave(double old, uint32_t utel, uint32_t unoe) {
	...
	if (unoe == 0) {
		...
		return (old * stat_damping +
				(dtel / dnoe) * (100 - stat_damping)) / 100;
	}
}
...
void stats_report() {
	...
	for (i = cp_up_dgram_sent = 0; i < serv_count; i++) {
		cp_up_dgram_sent += ar_up_dgram_sent[i] = meas_up_dgram_sent[i];
	}
	for (i = cp_up_ack_rcv = 0; i < serv_count; i++) {
		cp_up_ack_rcv += ar_up_ack_rcv[i] = meas_up_ack_rcv[i];
	}
	...
	/* Do the math */
	strftime(stat_timestamp, sizeof stat_timestamp, "%F %T %Z",
			 gmtime(&current_time));
	strftime(iso_timestamp, sizeof stat_timestamp, "%FT%TZ",
			 gmtime(&current_time));
	...
	pthread_mutex_lock(&mx_meas_dw);
	for (i = cp_dw_pull_sent = 0; i < serv_count; i++) {
		cp_dw_pull_sent += ar_dw_pull_sent[i] = meas_dw_pull_sent[i];
	}
	for (i = cp_dw_ack_rcv = 0; i < serv_count; i++) {
		cp_dw_ack_rcv += ar_dw_ack_rcv[i] = meas_dw_ack_rcv[i];
	}
	for (i = cp_dw_dgram_rcv = 0; i < serv_count; i++) {
		cp_dw_dgram_rcv += ar_dw_dgram_rcv[i] = meas_dw_dgram_rcv[i];
	}
	for (i = cp_dw_dgram_acp = 0; i < serv_count; i++) {
		cp_dw_dgram_acp += ar_dw_dgram_acp[i] = meas_dw_dgram_acp[i];
	}
	...
	for (i = 0; i < serv_count; i++) {
		move_up_ack_quality[i] =
			moveave(move_up_ack_quality[i], ar_up_ack_rcv[i],
					ar_up_dgram_sent[i]);
	}
	for (i = 0; i < serv_count; i++) {
		move_dw_ack_quality[i] =
			moveave(move_dw_ack_quality[i], ar_dw_ack_rcv[i],
					ar_dw_pull_sent[i]);
	}
	for (i = 0; i < serv_count; i++) {
		move_dw_datagram_quality[i] =
			moveave(move_dw_datagram_quality[i], ar_dw_dgram_acp[i],
					ar_dw_dgram_rcv[i]);
	}
	for (i = 0; i < serv_count; i++) {
		move_dw_receive_quality[i] =
			moveave(move_dw_receive_quality[i], ar_dw_ack_rcv[i],
					ar_dw_pull_sent[i]);
	}
	move_dw_beacon_quality =
		moveave(move_dw_beacon_quality, cp_nb_beacon_sent, cp_nb_beacon_queued);
	...
	if (gps_enabled == true) {
		...
		if (gps_fake_enable == true) {
			printf
				("# Manual GPS coordinates: latitude %.5f, longitude %.5f, altitude %i m\n",
				 cp_gps_coord.lat, cp_gps_coord.lon, cp_gps_coord.alt);
		} else if (coord_ok == true) {
			printf
				("# System GPS coordinates: latitude %.5f, longitude %.5f, altitude %i m\n",
				 cp_gps_coord.lat, cp_gps_coord.lon, cp_gps_coord.alt);
		} else {
	...
	if (upstream_enabled == true) {
		...
		for (i = 0; i < serv_count; i++) {
			if (servers[i].type == semtech && servers[i].upstream == true)
				printf
					("# Upstream datagram acknowledgment quality for server \"%s\" is %.2f%%.\n",
	...
	if (downstream_enabled == true) {
		for (i = 0; i < serv_count; i++) {
			if (servers[i].type == semtech && servers[i].downstream == true)
				printf
					("# Downstream heart beat acknowledgment quality for server \"%s\" is %.2f%%.\n",
					 servers[i].addr, 100 * move_dw_ack_quality[i]);
		}
		for (i = 0; i < serv_count; i++) {
			if (servers[i].type == semtech && servers[i].downstream == true)
				printf
					("# Downstream datagram content quality for server \"%s\" is %.2f%%.\n",
					 servers[i].addr, 100 * move_dw_datagram_quality[i]);
		}
		for (i = 0; i < serv_count; i++) {
			if (servers[i].type == semtech && servers[i].downstream == true)
				printf
					("# Downstream radio transmission quality for server \"%s\" is %.2f%%.\n",
	...
	if (statusstream_enabled == true || has_stat_file == true) {
		...
		JSON_Array *servers_array_object =
			json_value_get_array(servers_array_value);
		for (ic = 0; ic < serv_count; ic++) {
			...
			json_object_set_boolean(sub_object, "found",
									servers[ic].live == true);
			if (servers[ic].live == true)
				json_object_set_number(sub_object, "last_seen", stall_time[ic]);
			else
				json_object_set_string(sub_object, "last_seen", "never");
			json_array_append_value(servers_array_object, sub_value);
		}
		json_object_set_value(root_object_verbose, "servers",
							  servers_array_value);
		...
		json_object_dotset_boolean(root_object_verbose, "device.up_active",
								   upstream_enabled == true);
		json_object_dotset_boolean(root_object_verbose, "device.down_active",
								   downstream_enabled == true);
		json_object_dotset_number(root_object_verbose, "device.latitude",
								  cp_gps_coord.lat);
		json_object_dotset_number(root_object_verbose, "device.longitude",
								  cp_gps_coord.lon);
		json_object_dotset_number(root_object_verbose, "device.altitude",
								  cp_gps_coord.alt);
		json_object_dotset_number(root_object_verbose, "device.uptime",
								  current_time - startup_time);
		json_object_dotset_string(root_object_verbose, "device.gps", gps_state);
		json_object_dotset_string(root_object_verbose, "device.platform",
								  platform);
		json_object_dotset_string(root_object_verbose, "device.email", email);
		json_object_dotset_string(root_object_verbose, "device.description",
								  description);
		json_object_dotset_number(root_object_verbose,
								  "current.up_radio_packets_received",
								  cp_nb_rx_rcv);
		json_object_dotset_number(root_object_verbose,
								  "current.up_radio_packets_crc_good",
								  cp_nb_rx_ok);
		json_object_dotset_number(root_object_verbose,
								  "current.up_radio_packets_crc_bad",
								  cp_nb_rx_bad);
		json_object_dotset_number(root_object_verbose,
								  "current.up_radio_packets_crc_absent",
								  cp_nb_rx_nocrc);
		json_object_dotset_number(root_object_verbose,
								  "current.up_radio_packets_dropped",
								  cp_nb_rx_drop);
		json_object_dotset_number(root_object_verbose,
								  "current.up_radio_packets_forwarded",
								  cp_up_pkt_fwd);
		json_object_dotset_int_array(root_object_verbose,
									 "current.up_server_datagrams_send",
									 ar_up_dgram_sent, serv_count);
		json_object_dotset_int_array(root_object_verbose,
									 "current.up_server_datagrams_acknowledged",
									 ar_up_ack_rcv, serv_count);
		json_object_dotset_int_array(root_object_verbose,
									 "current.down_heartbeat_send",
									 ar_dw_pull_sent, serv_count);
		json_object_dotset_int_array(root_object_verbose,
									 "current.down_heartbeat_received",
									 ar_dw_ack_rcv, serv_count);
		json_object_dotset_int_array(root_object_verbose,
									 "current.down_server_datagrams_received",
									 ar_dw_dgram_rcv, serv_count);
		json_object_dotset_int_array(root_object_verbose,
									 "current.down_server_datagrams_accepted",
									 ar_dw_dgram_acp, serv_count);
		json_object_dotset_number(root_object_verbose,
								  "current.down_radio_packets_succes",
								  cp_nb_tx_ok);
		json_object_dotset_number(root_object_verbose,
								  "current.down_radio_packets_failure",
								  cp_nb_tx_fail);
		json_object_dotset_number(root_object_verbose,
								  "current.down_radio_packets_collision_packet",
								  cp_nb_tx_rejected_collision_packet);
		json_object_dotset_number(root_object_verbose,
								  "current.down_radio_packets_collision_beacon",
								  cp_nb_tx_rejected_collision_beacon);
		json_object_dotset_number(root_object_verbose,
								  "current.down_radio_packets_too_early",
								  cp_nb_tx_rejected_too_early);
		json_object_dotset_number(root_object_verbose,
								  "current.down_radio_packets_too_late",
								  cp_nb_tx_rejected_too_late);
		json_object_dotset_number(root_object_verbose,
								  "current.down_beacon_packets_queued",
								  cp_nb_beacon_queued);
		json_object_dotset_number(root_object_verbose,
								  "current.down_beacon_packets_send",
								  cp_nb_beacon_sent);
		json_object_dotset_number(root_object_verbose,
								  "current.down_beacon_packets_rejected",
								  cp_nb_beacon_rejected);
		json_object_dotset_number(root_object_verbose,
								  "performance.up_radio_packet_quality",
								  move_up_rx_quality);
		json_object_dotset_double_array(root_object_verbose,
										"performance.up_server_datagram_quality",
										move_up_ack_quality, serv_count);
		json_object_dotset_double_array(root_object_verbose,
										"performance.down_server_heartbeat_quality",
										move_dw_ack_quality, serv_count);
		json_object_dotset_double_array(root_object_verbose,
										"performance.down_server_datagram_quality",
										move_dw_datagram_quality, serv_count);
		json_object_dotset_double_array(root_object_verbose,
										"performance.down_radio_packet_quality",
										move_dw_receive_quality, serv_count);
		json_object_dotset_number(root_object_verbose,
								  "performance.down_beacon_packet_quality",
								  move_dw_beacon_quality);
	}
	if (statusstream_enabled == true && lorank_idee_concise) {
		...
		json_object_dotset_number(root_object_concise, "dev.lat",
								  cp_gps_coord.lat);
		json_object_dotset_number(root_object_concise, "dev.lon",
								  cp_gps_coord.lon);
		json_object_dotset_number(root_object_concise, "dev.alt",
								  cp_gps_coord.alt);
		json_object_dotset_number(root_object_concise, "dev.up",
								  current_time - startup_time);
		...
		if (upstream_enabled == true) {
			json_object_dotset_number(root_object_concise, "prf.up_rf",
									  move_up_rx_quality);
			json_object_dotset_double_array(root_object_concise,
											"prf.up_srv_dg",
											move_up_ack_quality, serv_count);
		}
		if (downstream_enabled == true) {
			...
			json_object_dotset_double_array(root_object_concise,
											"prf.dw_srv_dg",
											move_dw_datagram_quality,
											serv_count);
			json_object_dotset_double_array(root_object_concise, "prf.dw_rf",
											move_dw_receive_quality,
											serv_count);
			json_object_dotset_number(root_object_concise, "prf.dw_bcn",
									  move_dw_beacon_quality);
		}
	}
	if (has_stat_file == true) {
		if (json_serialize_to_file_pretty(root_value_verbose, stat_file_tmp) ==
			JSONSuccess)
	...
	transport_status_up(cp_nb_rx_rcv, cp_nb_rx_ok, cp_nb_tx_ok + cp_nb_tx_fail,
						cp_nb_tx_ok);
}
