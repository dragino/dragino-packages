...
int get_concentrator_time(struct timeval *concent_time,
	...
	MSG_DEBUG(DEBUG_TIMERSYNC, " --> TIME: unix current time is   %ld,%ld\n",
			  (long)unix_time.tv_sec, (long)unix_time.tv_usec);
	MSG_DEBUG(DEBUG_TIMERSYNC, "           offset is              %ld,%ld\n",
			  (long)offset_unix_concent.tv_sec,
			  (long)offset_unix_concent.tv_usec);
	MSG_DEBUG(DEBUG_TIMERSYNC, "           sx1301 current time is %ld,%ld\n",
			  (long)local_timeval.tv_sec, (long)local_timeval.tv_usec);
...
void thread_timersync(void) {
	...
	while (!exit_sig && !quit_sig) {
		...
		concentrator_timeval.tv_usec =
			sx1301_timecount - (concentrator_timeval.tv_sec * 1000000UL);
		...
		MSG_DEBUG(DEBUG_TIMERSYNC,
				  ...
				  sx1301_timecount, (long)concentrator_timeval.tv_sec,
				  (long)concentrator_timeval.tv_usec);
		MSG_DEBUG(DEBUG_TIMERSYNC, "  unix_timeval = %ld,%ld\n",
				  (long)unix_timeval.tv_sec, (long)unix_timeval.tv_usec);
