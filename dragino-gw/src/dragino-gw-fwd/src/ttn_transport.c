...
void ttn_downlink(Router__DownlinkMessage * msg, __attribute__ ((unused)) void *arg) {
	...
	if (msg->has_payload) {
		...
		case PROTOCOL__TX_CONFIGURATION__PROTOCOL_LORAWAN:{
				...
				j = snprintf((json + buff_index), 300 - buff_index,
							 ",\"frequency\":%d,\"rf_chain\":%d,\"rf_power\":%d",
				...
				if (lora->modulation == LORAWAN__MODULATION__LORA) {
					...
					if (strcmp(lora->coding_rate, "4/5") == 0)
						txpkt.coderate = CR_LORA_4_5;
					else if (strcmp(lora->coding_rate, "4/6") == 0)
						txpkt.coderate = CR_LORA_4_6;
					else if (strcmp(lora->coding_rate, "2/3") == 0)
						txpkt.coderate = CR_LORA_4_6;
					else if (strcmp(lora->coding_rate, "4/7") == 0)
						txpkt.coderate = CR_LORA_4_7;
					else if (strcmp(lora->coding_rate, "4/8") == 0)
						txpkt.coderate = CR_LORA_4_8;
					else if (strcmp(lora->coding_rate, "1/2") == 0)
						txpkt.coderate = CR_LORA_4_8;
					else {
				...
				memcpy(txpkt.payload, msg->payload.data,
					   txpkt.size < sizeof txpkt.payload ? txpkt.size : sizeof txpkt.  payload);
				j = snprintf((json + buff_index), 300 - buff_index,
							 ",\"length\":%d", txpkt.size);
				if (j > 0) {
					buff_index += j;
...
void ttn_dummy_downlink( __attribute__ ((unused)) Router__DownlinkMessage * msg,
...
void ttn_connect(int idx) {
	...
	while (!exit_sig && !quit_sig) {
		if (waittime == 0) {
			...
			if (debug_mask & DEBUG_PKT_FWD) {
				...
				while (t < waittime) {
					MSG_DEBUG(DEBUG_PKT_FWD,
							  "ttn_connect: sleeping() at %d, total %d\n", t,
							  waittime);
					sleep(10);
					t += 10;
				}
			...
			if (waittime < 300)
				waittime = 2 * waittime;	// Exponential backoff up to a maximum of 480 seconds
		}
		ttngwc_init(&servers[idx].ttn, servers[idx].gw_id,
					servers[idx].downstream ==
					true ? &ttn_downlink : &ttn_dummy_downlink, NULL);
		if (!servers[idx].ttn) {
			MSG("ERROR: [TTN] Initialize server \"%s\" failed, retry in %d seconds\n", servers[idx].addr, waittime);
			// Will this ever recover? Retry anyway...
			continue;
		}
		int err = ttngwc_connect(servers[idx].ttn, servers[idx].addr,
...
void ttn_upstream(void *pic) {
	...
	while (!exit_sig && !quit_sig) {
		...
		if ((servers[idx].live == true && err != 1)
			|| (servers[idx].connecting == false && err == 0)) {
...
void ttn_status_up(int idx, uint32_t rx_in, uint32_t rx_ok, uint32_t tx_in, uint32_t tx_ok) {
	...
	if (temp_available >= 0) {
		if (temp_available > 0
			|| access("/sys/class/thermal/thermal_zone0/temp", R_OK) == 0) {
			char buffer[20];
...
void ttn_status(int idx) {
	printf("# %s: %s\n", servers[idx].addr,
		   servers[idx].connecting ? 
           "Connecting" : !servers[idx].live ? 
           "Unknown status" : ttngwc_checkconnected(servers[idx].ttn) ? 
           "Connected" : "Broken connection");
}
