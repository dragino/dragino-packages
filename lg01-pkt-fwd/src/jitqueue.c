...
uint32_t time_on_air(struct pkt_tx_s *packet) {
    ...
    if (packet == NULL) {
        MSG_DEBUG(DEBUG_INFO, "ERROR: Failed to compute time on air, wrong parameter\n");
        return 0;
    }
...
enum jit_error_e jit_enqueue(struct jit_queue_s *queue, struct timeval *time, struct pkt_tx_s *packet, enum jit_pkt_type_e pkt_type) {
    int i = 0;
    ...
    switch (pkt_type) {
        ...
        case JIT_PKT_TYPE_DOWNLINK_CLASS_C:
            ...
            packet_post_delay = time_on_air(packet) * 1000UL; /* in us */
            break;
    ...
    memcpy(&(queue->nodes[queue->num_pkt].pkt), packet, sizeof(struct pkt_tx_s));
...
enum jit_error_e jit_dequeue(struct jit_queue_s *queue, int index, struct pkt_tx_s *packet, enum jit_pkt_type_e *pkt_type) {
    if (packet == NULL) {
    ...
    memcpy(packet, &(queue->nodes[index].pkt), sizeof(struct pkt_tx_s));
    queue->num_pkt--;
