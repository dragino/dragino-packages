...
int main(void)
{
    ...
    while (!exit_sig && !quit_sig) {
        ...
        if (gps_fake_enable == true) {
            ...
            /* display a report */
            fprintf(stdout, "\n################## Report at: %s ##################\n", stat_timestamp);
            fprintf(stdout, "### [UPSTREAM] ###\n");
            fprintf(stdout, "# RF packets received by concentrator: %u\n", cp_nb_rx_rcv);
            fprintf(stdout, "# CRC_OK: %.2f%%, CRC_FAIL: %.2f%%, NO_CRC: %.2f%%\n", 100.0 * rx_ok_ratio, 100.0 * rx_bad_ratio, 100.0 * rx_nocrc_ratio);
            fprintf(stdout, "# RF packets forwarded: %u (%u bytes)\n", cp_up_pkt_fwd, cp_up_payload_byte);
            fprintf(stdout, "# PUSH_DATA datagrams sent: %u (%u bytes)\n", cp_up_dgram_sent, cp_up_network_byte);
            fprintf(stdout, "# PUSH_DATA acknowledged: %.2f%%\n", 100.0 * up_ack_ratio);
            fprintf(stdout, "### [DOWNSTREAM] ###\n");
            fprintf(stdout, "# PULL_DATA sent: %u (%.2f%% acknowledged)\n", cp_dw_pull_sent, 100.0 * dw_ack_ratio);
            fprintf(stdout, "# PULL_RESP(onse) datagrams received: %u (%u bytes)\n", cp_dw_dgram_rcv, cp_dw_network_byte);
            fprintf(stdout, "# RF packets sent to concentrator: %u (%u bytes)\n", (cp_nb_tx_ok+cp_nb_tx_fail), cp_dw_payload_byte);
            fprintf(stdout, "# TX errors: %u\n", cp_nb_tx_fail);
            if (cp_nb_tx_requested != 0 ) {
                fprintf(stdout, "# TX rejected (collision packet): %.2f%% (req:%u, rej:%u)\n", 100.0 * cp_nb_tx_rejected_collision_packet / cp_nb_tx_requested, cp_nb_tx_requested, cp_nb_tx_rejected_collision_packet);
                fprintf(stdout, "# TX rejected (collision beacon): %.2f%% (req:%u, rej:%u)\n", 100.0 * cp_nb_tx_rejected_collision_beacon / cp_nb_tx_requested, cp_nb_tx_requested, cp_nb_tx_rejected_collision_beacon);
                fprintf(stdout, "# TX rejected (too late): %.2f%% (req:%u, rej:%u)\n", 100.0 * cp_nb_tx_rejected_too_late / cp_nb_tx_requested, cp_nb_tx_requested, cp_nb_tx_rejected_too_late);
                fprintf(stdout, "# TX rejected (too early): %.2f%% (req:%u, rej:%u)\n", 100.0 * cp_nb_tx_rejected_too_early / cp_nb_tx_requested, cp_nb_tx_requested, cp_nb_tx_rejected_too_early);
            }
            fprintf(stdout, "# BEACON queued: %u\n", cp_nb_beacon_queued);
            fprintf(stdout, "# BEACON sent so far: %u\n", cp_nb_beacon_sent);
            fprintf(stdout, "# BEACON rejected: %u\n", cp_nb_beacon_rejected);
            fprintf(stdout, "### [JIT] ###\n");
